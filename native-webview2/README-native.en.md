...
[Korean document](README-native.md)
...
The latest public v2 release is `v2.0.0-preview.7`. Current `main` also contains post-preview.7 updates: a selectable 256x256 animated GIF center logo, graph-fill animation while the animated logo is active, and a `Balloon notifications` On/Off menu that defaults to Off. Until another v2 preview release is created, the GitHub Release ZIP contains the preview.7 feature set.

The v2 preview has been verified with a real visible ChatGPT login, persistent WebView2 session, authenticated usage-page parsing, general Codex reset/credits parsing, opt-in automatic refresh, tray menu operation, color settings, dynamic tray icon rendering, Rings/Bars/Meters/Battery widget styles, cache cleanup, stable exit, and a clean GitHub Actions artifact. It is still labeled **preview** because it has no installer, startup registration, code signing, or broad multi-machine compatibility testing.
...
- The released `v2.0.0-preview.7` package reports preview.7 in About and file metadata. The current source may include unreleased post-preview.7 changes until the next preview tag is created.
...
- The widget keeps its taskbar-hidden tool-window behavior while making a best-effort attempt to pin itself across Windows virtual desktops.
...
When enabled, the tooltip includes the interval and next scheduled time. Automatic collection periodically loads the usage page with the local WebView2 session previously created by the user. Normal automatic loading does not use automation-evasion flags, extracted cookies, token calls, or external servers. If the session has expired or authentication is required, `Fetch now` and auto refresh report the login-required state without automatically showing a page. Use `Open/Login usage page` when a visible sign-in page is needed. The app never supplies its own login form.
...
- There is no automation-detection bypass, cookie extraction, unofficial token call, external relay, or bundled Chromium. Background fetch uses the app's local WebView2 session and reports login-required states instead of opening a login page automatically.
