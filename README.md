...
Python을 직접 설치하지 않고 사용하려면 GitHub 저장소의 [Releases](https://github.com/saveway/codex-usage-monitor/releases)에서 다음 두 파일을 내려받습니다.
...
GitHub Actions에서 artifact를 내려받으면 `CodexUsageMonitor-windows-full`이라는 바깥쪽 ZIP 안에 실제 배포 파일 `CodexUsageMonitor-windows-full.zip`과 `CodexUsageMonitor-windows-full.zip.sha256`이 들어 있습니다. 바깥쪽 ZIP을 먼저 푼 다음 안쪽 배포 ZIP의 SHA256을 확인하십시오. GitHub Release에서는 이 두 파일을 직접 내려받습니다.
...
수동으로 실행한 Actions 빌드는 저장소의 Actions 실행 화면에서 `CodexUsageMonitor-windows-full` artifact로 내려받을 수 있습니다. `v*` 태그 빌드는 같은 ZIP과 SHA256 파일을 GitHub Release에 자동 첨부합니다. Chromium을 포함하지 않는 더 작은 `lite` 배포본은 현재 제공하지 않으며, 향후 별도 Python/Playwright 설치를 전제로 추가할 수 있습니다.
