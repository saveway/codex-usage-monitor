...
v2는 v1을 대체하는 정식 기본 배포가 아닙니다. 자세한 실행·보안·제한사항은 [WebView2 native preview 문서](native-webview2/README-native.md)를 참고하십시오. [영문 v2 문서](native-webview2/README-native.en.md)도 별도로 보관되어 있습니다.

v2 Preview는 installer가 아닌 portable ZIP 배포본입니다. Rings, Bars, Meters, Battery 위젯 그래프, reset/credits 표시, zero alert, 색상 설정, 자동 조회, WebView2 기반 숨김 조회를 제공합니다. 현재 `main`에는 preview.7 이후 256x256 animated GIF 로고, 그래프 채움 애니메이션, balloon 알림 On/Off 메뉴도 반영되어 있습니다. 새 v2 Release가 만들어지기 전까지 GitHub Release ZIP은 preview.7 기준 기능을 제공합니다.

ZIP 안에서 바로 실행하거나 임시 폴더에서 실행하지 말고, 먼저 계속 사용할 영구 폴더에 전체 압축을 푼 뒤 `CodexUsageMonitorV2.exe`를 실행하십시오. 임시 위치에서 실행하면 재실행, 경로 유지 또는 향후 자동 시작 구성에 문제가 생길 수 있습니다.
...
아래 이미지는 실제 계정 정보가 아닌 예제 값으로 만든 공개용 화면입니다.

### v2 Rings 위젯

![예제 사용량과 animated 로고를 표시하는 v2 Rings 위젯](docs/images/widget-rings.png)

### v2 위젯 그래프 스타일

| Bars | Meters | Battery |
|---|---|---|
| ![Bars 위젯](docs/images/widget-bars.png) | ![Meters 위젯](docs/images/widget-meters.png) | ![Battery 위젯](docs/images/widget-battery.png) |

### v2 트레이 및 위젯 우클릭 메뉴

![트레이와 위젯에서 공통으로 사용하는 우클릭 메뉴](docs/images/tray-menu.png)

### 그래프 스타일 선택

![Rings, Bars, Meters, Battery 그래프 스타일 메뉴](docs/images/graph-style-menu.png)

### 센터 로고와 알림 설정

| Center logo | Balloon notifications |
|---|---|
| ![정적 아이콘 또는 256 전용 animated GIF를 선택하는 메뉴](docs/images/center-logo-menu.png) | ![Windows balloon 알림 토글 메뉴](docs/images/balloon-notifications-menu.png) |
...
## v1 Stable GitHub Release에서 Windows 배포본 실행

[v1.0.1 Stable Release](https://github.com/saveway/codex-usage-monitor/releases/tag/v1.0.1)에서 Full 또는 Lite 배포본과 같은 이름의 SHA256 파일을 내려받습니다. 더 작은 v2 preview를 찾는다면 위의 [권장 선택](#권장-선택)을 먼저 확인하십시오.
...
수동으로 실행한 Actions 빌드는 저장소의 Actions 실행 화면에서 `CodexUsageMonitor-windows-full`과 `CodexUsageMonitor-windows-lite` artifact로 내려받을 수 있습니다. `v1.*` 태그 빌드는 Full/Lite ZIP과 각 SHA256 파일을 GitHub Release에 자동 첨부합니다.
...
브라우저 로그인 세션을 유지하는 과정에서 `browser-profile/`의 캐시 때문에 `%LOCALAPPDATA%\CodexUsageMonitor` 용량이 커질 수 있습니다. 프로그램은 시작과 수집 완료 후 세션에 필요하지 않은 브라우저 캐시를 정리하고 로그 크기를 제한하지만, 장기간 사용하면 일부 브라우저 데이터가 남을 수 있습니다.

완전히 초기화하려면 트레이 또는 위젯 메뉴에서 `Exit`로 앱을 종료한 뒤 `%LOCALAPPDATA%\CodexUsageMonitor` 폴더 전체를 삭제하십시오. 사용량, 설정, 로그와 저장된 로그인 세션이 모두 삭제되므로 다음 실행 시 ChatGPT 로그인이 다시 필요합니다.
