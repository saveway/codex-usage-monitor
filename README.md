...
4. 압축을 푼 폴더의 `run-codex-usage-tray.bat`를 실행합니다. 배포본에는 PyInstaller로 만든 스크래퍼 exe와 Playwright Chromium이 포함되어 있으므로 Python을 별도로 설치할 필요가 없습니다.
...
수동으로 실행한 Actions 빌드는 저장소의 Actions 실행 화면에서 `CodexUsageMonitor-windows` artifact로 내려받을 수 있습니다. `v*` 태그 빌드는 같은 ZIP과 SHA256 파일을 GitHub Release에 자동 첨부합니다.
